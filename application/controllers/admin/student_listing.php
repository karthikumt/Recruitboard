...
class Student_listing extends CI_Controller {
	...
	public function total_user($type){
		echo $this->admin_model->get_user_count($type);
	}
	public function view_student_more($start=0){
		
		$data['user_list'] = $this->admin_model->get_user_data_limit('user_register', 'UserType', 1, $start);
		$d = $this->load->view('admin/view_student_more', $data, true);
        echo $d;
	}
