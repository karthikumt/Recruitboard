			...
			if($LoginCheck['UserType']==1 || $LoginCheck['UserType']==2 )
			 {
				if($LoginCheck['PaymentType']=="1")
				{
					if($LoginCheck['Payment']!="")
					{
						...
						$days=$diff->format("%a%");
							...
							{
								if($LoginCheck['IsCancel']==1)
								{
									$this->general_model->update_data('user_register',array('PaymentType'=>0),array('UserID'=>$LoginCheck['UserID']));
									redirect(base_url().'dashboard');
								}/*else{
				...
				if($LoginCheck['PaymentType']=="2")
				{
					...
					if($LoginCheck['UserType']==1)
					{
						...
						{
							if($LoginCheck['Payment']=="" && $LoginCheck['PaymentType']!='0')
							{
